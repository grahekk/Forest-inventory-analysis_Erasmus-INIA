---
title: "Calculo de inventarios de Lubia"
...
Lubia <- read_csv2("INVENTARIOS_1_2_3_BDD_LUBIA_NS_cambiado.csv")

#la variabla que contiene solo pinos
Lubia %>% filter(ESPECIE == "Pinus pinaster") -> Lubia_pino
#y variabla que contiene solo rebollo
Lubia %>% filter(ESPECIE != "Pinus pinaster") -> Lubia_rebollo
...
Lubia_pino %>% group_by(PARCELA, `INVENTARIO`) %>% 
  count(`TIPO MASA`) %>% 
  pivot_wider(names_from = c(`INVENTARIO`, `TIPO MASA`), values_from = "n") -> Lubia_inventario_pino

Lubia_inventario_pino %>% kbl(caption = "<center><strong>Lubia - Pino</strong></center>",escape = FALSE) %>% kable_classic()
...
Lubia_rebollo %>% 
  ...
  count(`TIPO MASA`) %>% 
  pivot_wider(names_from = c(`INVENTARIO`, `TIPO MASA`), values_from = "n") -> Lubia_inventario_rebollo

Lubia_inventario_rebollo %>% kbl(caption = "<center><strong>Lubia - Rebollo</strong></center>",escape = FALSE) %>% kable_classic()
...
#superficies de tabla de archivo "informe Lubia ......."
Superficie_Lubia <- c(0.079,0.085,0.080,0.076,0.081,0.078,0.083,0.081,0.081)
...
Modelos lineares hice en sus proprios scripts que se llaman **Lubia_pino_1_modelos_lineares.r** y **Lubia_rebollo_1_modelos_lineares.r**. Estos dos despues he encargado en el codigo principial, de este manera el codigo principial es mas limpio y en orden.
...
Lubia_pino %>% filter(is.na(`ALT - TOTAL`)==F) -> var1

Lubia_rebollo %>% filter(is.na(`ALT - TOTAL`)==F) -> var3
...
e_pino <- new.env()
source("Lubia_pino_1_modelos_lineares.r", local = e_pino)
attach(e_pino)

#llenacion de valores que faltan en Altura_pred y Altura_def, pino
Lubia_pino %>% mutate(Altura_def = ifelse(is.na(`ALT - TOTAL`), var2$Prediccion, `ALT - TOTAL`), Altura_pred = var2$Prediccion) -> Lubia_H_pred_pino

e_rebollo <- new.env()
source("Lubia_rebollo_1_modelos_lineares.r", local = e_rebollo)
attach(e_rebollo)

#llenacion de valores que faltan en Altura_pred y Altura_def, rebollo
#tenia que sortir variablas var4 y Lubia_rebollo porque no eran alineadas
var4 %>% arrange(INVENTARIO, PARCELA, `NO PIE`) -> var4

Lubia_rebollo %>% arrange(INVENTARIO, PARCELA, `NO PIE`) %>% mutate(Altura_def = ifelse(is.na(`ALT - TOTAL`), var4$Prediccion, `ALT - TOTAL`), Altura_pred = var4$Prediccion) -> Lubia_H_pred_rebollo
...
Archivos se llaman: **Lubia_pino_2_grafica_diam-alt.r** y **Lubia_rebollo_2_grafica_diam-alt.r**
...
Lubia_H_pred_rebollo$INVENTARIO <- as.factor(Lubia_H_pred_rebollo$INVENTARIO)

#aqí es un ejemplo de grafica hecha tras ggplot2 package
Lubia_H_pred_rebollo %>% filter(PARCELA==5) %>%  
...
#tengo que cambiar EDAD en factor para hacerlo variable discrete
Lubia_H_pred_pino$INVENTARIO <- as.factor(Lubia_H_pred_pino$INVENTARIO)
Lubia_H_pred_rebollo$INVENTARIO <- as.factor(Lubia_H_pred_rebollo$INVENTARIO)

source("Lubia_pino_2_grafica_diam-alt.r")
gda1
gda2
gda3
gda4
...
source("Lubia_rebollo_2_grafica_diam-alt.r")
gda1
gda2
gda3
gda4
...
Lubia_pino %>% 
  ...
  transmute(Nac = sum(`E`,`NA`, na.rm = T), Ndc = `NA`, Ne = `E`) -> Lubia_P

#Superficie
Lubia_P$Superficie <- rep(Superficie_Lubia, each=3)

#Calculo de numero de pies por hectárea
Lubia_P[is.na(Lubia_P)] <- 0
Lubia_P %>% mutate(Superficie = Superficie, 
                      Nac = Nac/Superficie, 
                      Ndc = Ndc/Superficie, 
                      Ne = Ne/Superficie) %>% round -> Lubia_P
...
Lubia_pino %>% 
  ...
  ungroup %>% select(Dgac) %>% unlist -> Lubia_P$Dgac
...
Lubia_pino %>% 
  ...
  ungroup %>% select(Dgdc) %>% unlist -> Lubia_P$Dgdc

#Dge
#primero he hecho la variabla en Lubia_H_pred_pino que contuvo solo diametros de arboles extraidos, de este manera es mas facil y limpio calcular medidas extraidos después
Lubia_H_pred_pino %>% 
  mutate(Diam_e = ifelse(`TIPO MASA`=="E", `DIAM MED`, NA)) -> Lubia_H_pred_pino

#calculo de Dge
Lubia_H_pred_pino %>% 
  group_by(PARCELA, INVENTARIO) %>% 
  summarise(Dge = dg(Diam_e/10)) %>% 
  ungroup %>% select(Dge) %>% unlist -> Lubia_P$Dge
...
Lubia_pino %>% 
  ...
  ungroup %>% select(Doac) %>% unlist -> Lubia_P$Doac
...
Lubia_H_pred_pino %>% 
  ...
  ungroup %>% select(Hgdc) %>% unlist -> Lubia_P$Hgdc

#Hge
#lo mismo como Dge, primero añadí una variabla en Lubia_H_pred_pino
Lubia_H_pred_pino %>% 
  mutate(Altura_e = ifelse(`TIPO MASA`=="E", Altura_def, NA)) -> Lubia_H_pred_pino

Lubia_H_pred_pino %>% 
  group_by(PARCELA, INVENTARIO) %>% 
  summarise(Hge = mean(Altura_e, na.rm = T)) %>% 
  ungroup %>% select(Hge) %>% unlist -> Lubia_P$Hge
...
Lubia_H_pred_pino %>% 
  ...
  ungroup %>% select(Hoac) %>% unlist -> Lubia_P$Hoac
...
Lubia_H_pred_pino %>% 
  ...
  ungroup %>% select(Hodc) %>% unlist -> Lubia_P$Hodc
...
Lubia_rebollo %>% 
  ...
  count(`TIPO MASA`) %>% 
  pivot_wider(names_from = c(`TIPO MASA`), values_from = "n") %>% 
  transmute(Nac = sum(`E`,`NA`, na.rm = T), Ndc = sum(`NA`, `N`, na.rm=T), Ne = `E`, Nn = `N`) -> Lubia_R

#Superficie
Lubia_R$Superficie <- rep(Superficie_Lubia, each=3)[c(-29,-32,-33)]

#Calculo de numero de pies por hectárea
Lubia_R[is.na(Lubia_R)] <- 0
Lubia_R %>% transmute(Superficie = Superficie, 
                         Nac = Nac/Superficie, 
                         Ndc = Ndc/Superficie, 
                         Ne = Ne/Superficie,
                         Nn = Nn/Superficie) %>% round -> Lubia_R
...
Lubia_rebollo %>% 
  group_by(PARCELA, INVENTARIO) %>% 
  summarise(Dgac = dg(`DIAM MED`)/10) %>% 
  ungroup %>% select(Dgac) %>% unlist -> Lubia_R$Dgac
...
Lubia_rebollo %>% 
  ...
  group_by(PARCELA, INVENTARIO) %>% 
  summarise(Dgdc = dg(`DIAM MED`/10)) %>% 
  ungroup %>% select(Dgdc) %>% unlist -> Lubia_R$Dgdc

#Dge
#Diam_e
Lubia_H_pred_rebollo %>% 
  mutate(Diam_e = ifelse(`TIPO MASA`=="E", `DIAM MED`, NA)) -> Lubia_H_pred_rebollo

Lubia_H_pred_rebollo %>% 
  group_by(PARCELA, INVENTARIO) %>% 
  summarise(Dge = dg(Diam_e/10)) %>% 
  ungroup %>% select(Dge) %>% unlist -> Lubia_R$Dge
...
Lubia_rebollo %>% 
  ...
  slice_max(n=7, order_by = `DIAM MED`) %>% 
  summarise(Doac = mean(`DIAM MED`/10)) %>% 
  ungroup %>% select(Doac) %>% unlist -> Lubia_R$Doac
...
Lubia_rebollo %>% 
  ...
  slice_max(n=7, order_by = `DIAM MED`) %>% 
  summarise(Dodc = mean(`DIAM MED`/10)) %>% 
  ungroup %>% select(Dodc) %>% unlist -> Lubia_R$Dodc

#Hgac
Lubia_H_pred_rebollo %>% 
  group_by(PARCELA, INVENTARIO) %>% 
  summarise(Hgac = mean(`ALT - TOTAL`, na.rm = T)) %>% 
  ungroup %>% select(Hgac) %>% unlist -> Lubia_R$Hgac
...
Lubia_H_pred_rebollo %>% 
  ...
  ungroup %>% select(Hgdc) %>% unlist -> Lubia_R$Hgdc

#Hge
#Altura_e
Lubia_H_pred_rebollo %>% 
  mutate(Altura_e = ifelse(`TIPO MASA`=="E", Altura_def, NA)) -> Lubia_H_pred_rebollo

Lubia_H_pred_rebollo %>% 
  group_by(PARCELA, INVENTARIO) %>% 
  summarise(Hge = mean(Altura_e, na.rm = T)) %>% 
  ungroup %>% select(Hge) %>% unlist -> Lubia_R$Hge
...
Lubia_H_pred_rebollo %>% 
  ...
  ungroup %>% select(Hoac) %>% unlist -> Lubia_R$Hoac
...
Lubia_H_pred_rebollo %>% 
  ...
  ungroup %>% select(Hodc) %>% unlist  -> Lubia_R$Hodc
...
Lubia_H_pred_pino %>% 
  ...
  mutate(AreaBasimetrica_e = ifelse(`TIPO MASA`=="E", AreaBasimetrica, NA)) -> Lubia_H_pred_pino

#ABac
Lubia_H_pred_pino %>% 
  group_by(PARCELA, INVENTARIO) %>% 
  summarise(ABac = sum(AreaBasimetrica)) %>% 
  ungroup %>% select(ABac) %>% unlist -> Lubia_P$ABac
...
Lubia_H_pred_pino %>% 
  ...
  ungroup %>% select(ABdc) %>% unlist -> Lubia_P$ABdc

#ABe
Lubia_H_pred_pino %>% 
  group_by(PARCELA, INVENTARIO) %>% 
  summarise(ABe = sum(AreaBasimetrica_e, na.rm = T)) %>% 
  ungroup %>% select(ABe) %>% unlist -> Lubia_P$ABe

#ABac, ABdc, ABe
Lubia_P %>% mutate(ABac = ABac/Superficie, 
                      ABdc = ABdc/Superficie, 
                      ABe = ABe/Superficie) -> Lubia_P

#ABT
Lubia_P %>% group_by(PARCELA) %>% 
  mutate(ABT = ABac + cumsum(ABe)-ABe) %>% 
  ungroup %>% select(ABT) %>% unlist -> Lubia_P$ABT
...
Lubia_H_pred_rebollo %>% 
  mutate(AreaBasimetrica = g(`DIAM MED`)) %>% 
  mutate(AreaBasimetrica_e = ifelse(`TIPO MASA`=="E", AreaBasimetrica, NA)) -> Lubia_H_pred_rebollo

#ABac
Lubia_H_pred_rebollo %>% 
  group_by(PARCELA, INVENTARIO) %>% 
  summarise(ABac = sum(`AreaBasimetrica`, na.rm = T)) %>% 
  ungroup %>% select(ABac) %>% unlist -> Lubia_R$ABac
...
Lubia_H_pred_rebollo %>% 
  ...
  ungroup %>% select(ABdc) %>% unlist -> Lubia_R$ABdc

#ABe
Lubia_H_pred_rebollo %>% 
  group_by(PARCELA, INVENTARIO) %>% 
  summarise(ABe = sum(AreaBasimetrica, na.rm = T)) %>% 
  ungroup %>% select(ABe) %>% unlist -> Lubia_R$ABe

#ABac, ABdc, ABe
Lubia_R %>% mutate(ABac = ABac/Superficie, 
                      ABdc = ABdc/Superficie, 
                      ABe = ABe/Superficie) -> Lubia_R

#ABT
Lubia_R %>% 
  group_by(PARCELA) %>% 
  mutate(ABT = ABac + cumsum(ABe)-ABe) %>% 
  ungroup %>% select(ABT) %>% unlist -> Lubia_R$ABT
...
Lubia_H_pred_pino %>% 
  ...
  mutate(Volumen_e = ifelse(`TIPO MASA`=="E", Volumen, NA)) -> Lubia_H_pred_pino

#Vac
Lubia_H_pred_pino %>% 
  group_by(PARCELA, INVENTARIO) %>% 
  summarise(Vac = sum(`Volumen`, na.rm = T)) %>% 
  ungroup %>% select(Vac) %>% unlist -> Lubia_P$Vac
...
Lubia_H_pred_pino %>% 
  ...
  ungroup %>% select(Vdc) %>% unlist -> Lubia_P$Vdc

#Ve
Lubia_H_pred_pino %>%
  group_by(PARCELA, INVENTARIO) %>% 
  summarise(Ve = sum(Volumen_e, na.rm = T)) %>% 
  ungroup %>% select(Ve) %>% unlist -> Lubia_P$Ve

#Vac, Vdc, Ve
Lubia_P %>% mutate(Vac = Vac/Superficie, 
                      Vdc = Vdc/Superficie, 
                      Ve = Ve/Superficie) -> Lubia_P

#VT
Lubia_P %>% 
  group_by(PARCELA) %>% 
  mutate(VT = Vac + cumsum(Ve)-Ve) %>% 
  ungroup %>% select(VT) %>% unlist -> Lubia_P$VT

#Vmac, Vmdc, Vme
Lubia_P %>% mutate(Vmac = Vac/Nac, 
                      Vmdc = Vdc/Ndc, 
                      Vme = Ve/Ne) -> Lubia_P
...
Lubia_H_pred_rebollo %>% 
  ...
  mutate(Volumen_e = ifelse(`TIPO MASA`=="E", Volumen, NA)) -> Lubia_H_pred_rebollo

#Vac
Lubia_H_pred_rebollo %>% 
  group_by(PARCELA, INVENTARIO) %>% 
  summarise(Vac = sum(`Volumen`, na.rm = T)) %>% 
  ungroup %>% select(Vac) %>% unlist -> Lubia_R$Vac
...
Lubia_H_pred_rebollo %>% 
  ...
  group_by(PARCELA, INVENTARIO) %>% 
  summarise(Vdc = sum(Volumen, na.rm = T)) %>% 
  ungroup %>% select(Vdc) %>% unlist -> Lubia_R$Vdc

#Ve
Lubia_H_pred_rebollo %>% 
  group_by(PARCELA, INVENTARIO) %>% 
  summarise(Ve = sum(Volumen_e, na.rm = T)) %>% 
  ungroup %>% select(Ve) %>% unlist -> Lubia_R$Ve

#Vac, Vdc, Ve
Lubia_R %>% mutate(Vac = Vac/Superficie, 
                   Vdc = Vdc/Superficie, 
                   Ve = (Ve/Superficie)) -> Lubia_R

#VT
Lubia_R %>% 
  group_by(PARCELA) %>% 
  mutate(VT = Vac + cumsum(Ve)-Ve) %>% 
  ungroup %>% select(VT) %>% unlist -> Lubia_R$VT

#Vmac, Vmdc, Vme
Lubia_R %>% mutate(Vmac = Vac/Nac, 
                   Vmdc = Vdc/Ndc, 
                   Vme = Ve/Ne) -> Lubia_R
...
Lubia_H_pred_pino %>% 
  ...
  mutate(Biomasa_s_e = ifelse(`TIPO MASA`=="E", Biomasa_s, NA)) -> Lubia_H_pred_pino

#Wb7_Wb27_pino, biomasa de ramas 7
Lubia_H_pred_pino %>% 
  mutate(Biomasa_b7_b27 = Wb7_Wb27_pino(`DIAM MED`)) %>%  
  mutate(Biomasa_b7_b27_e = ifelse(`TIPO MASA`=="E", Biomasa_b7_b27, NA)) -> Lubia_H_pred_pino

#Wb2_n_pino, biomasa de ramas, ramitas
Lubia_H_pred_pino %>% 
  mutate(Biomasa_b2_n = Wb2_n_pino(`DIAM MED`)) %>% 
  mutate(Biomasa_b2_n_e = ifelse(`TIPO MASA`=="E", Biomasa_b2_n, NA)) -> Lubia_H_pred_pino
...
Lubia_H_pred_pino %>% 
  ...
  ungroup %>% select(Wac) %>% unlist -> Lubia_P$Wac
...
Lubia_H_pred_pino %>% 
  ...
  ungroup %>% select(Wdc) %>% unlist -> Lubia_P$Wdc

#We_s
Lubia_H_pred_pino %>%  
  group_by(PARCELA, INVENTARIO) %>% 
  summarise(We = sum(Biomasa_s_e, na.rm = T)) %>% 
  ungroup %>% select(We) %>% unlist -> Lubia_P$We
...
Lubia_H_pred_pino%>% 
  ...
  ungroup %>% select(Wac_b7_b27) %>% unlist -> Lubia_P$Wac_b7_b27
...
Lubia_H_pred_pino %>%
  ...
  ungroup %>% select(Wdc_b7_b27) %>% unlist -> Lubia_P$Wdc_b7_b27

#We_b7_b27
Lubia_H_pred_pino %>% 
  group_by(PARCELA, INVENTARIO) %>% 
  summarise(We_b7_b27 = sum(Biomasa_b7_b27_e, na.rm = T)) %>% 
  ungroup %>% select(We_b7_b27) %>% unlist -> Lubia_P$We_b7_b27

#Wac_b2_n
Lubia_H_pred_pino %>% 
  group_by(PARCELA, INVENTARIO) %>% 
  summarise(Wac_b2_n = sum(Biomasa_b2_n, na.rm = T)) %>% 
  ungroup %>% select(Wac_b2_n) %>% unlist -> Lubia_P$Wac_b2_n
...
Lubia_H_pred_pino %>% 
  ...
  ungroup %>% select(Wdc_b2_n) %>% unlist -> Lubia_P$Wdc_b2_n

#We_b2_n
Lubia_H_pred_pino %>% 
  group_by(PARCELA, INVENTARIO) %>% 
  summarise(We_b2_n = sum(Biomasa_b2_n_e, na.rm = T)) %>% 
  ungroup %>% select(We_b2_n) %>% unlist -> Lubia_P$We_b2_n

#WT
Lubia_P %>% 
  mutate(WacT = sum(Wac, Wac_b7_b27, Wac_b2_n),
         WdcT = sum(Wdc, Wdc_b7_b27, Wdc_b2_n),
         WeT = sum(We, We_b7_b27, We_b2_n)) -> Lubia_P
...
Lubia_H_pred_rebollo %>% 
  ...
  mutate(Biomasa_s_7_e = ifelse(`TIPO MASA`=="E", Biomasa_s_7, NA)) -> Lubia_H_pred_rebollo

#Wb2_7_rebollo, biomasa de ramas 2-7
Lubia_H_pred_rebollo %>% 
  mutate(Biomasa_b2_7 = Wb2_7_rebollo(`DIAM MED`, Altura_def)) %>% 
  mutate(Biomasa_b2_7_e = ifelse(`TIPO MASA`=="E", Biomasa_b2_7, NA)) -> Lubia_H_pred_rebollo

#Wb2_rebollo, biomasa de ramas, ramitas
Lubia_H_pred_rebollo %>% 
  mutate(Biomasa_b2_n = Wb2_rebollo(`DIAM MED`, Altura_def)) %>% 
  mutate(Biomasa_b2_n_e = ifelse(`TIPO MASA`=="E", Biomasa_b2_n, NA)) -> Lubia_H_pred_rebollo
...
Lubia_H_pred_rebollo %>% 
  ...
  ungroup %>% select(Wac) %>% unlist -> Lubia_R$Wac_s_7
...
Lubia_H_pred_rebollo %>% 
  ...
  ungroup %>% select(Wdc) %>% unlist -> Lubia_R$Wdc_s_7

#We_s_7
Lubia_H_pred_rebollo %>%  
  group_by(PARCELA, INVENTARIO) %>% 
  summarise(We = sum(Biomasa_s_7_e, na.rm = T)) %>% 
  ungroup %>% select(We) %>% unlist -> Lubia_R$We_s_7
...
Lubia_H_pred_rebollo%>% 
  ...
  ungroup %>% select(Wac_b2_7) %>% unlist -> Lubia_R$Wac_b2_7
#Wdc_b2_7
Lubia_H_pred_rebollo %>% 
  filter(is.na(`TIPO MASA`)==T|`TIPO MASA`=="N") %>% 
  group_by(PARCELA, INVENTARIO) %>% 
  summarise(Wdc_b2_7 = sum(Biomasa_b2_7, na.rm = T)) %>% 
  ungroup %>% select(Wdc_b2_7) %>% unlist -> Lubia_R$Wdc_b2_7

#We_b2_7
Lubia_H_pred_rebollo %>%
  group_by(PARCELA, INVENTARIO) %>% 
  summarise(We_b2_7 = sum(Biomasa_b2_7_e, na.rm = T)) %>% 
  ungroup %>% select(We_b2_7) %>% unlist -> Lubia_R$We_b2_7

#Wac_b2_n
Lubia_H_pred_rebollo %>% 
  group_by(PARCELA, INVENTARIO) %>% 
  summarise(Wac_b2_n = sum(Biomasa_b2_n, na.rm = T)) %>% 
  ungroup %>% select(Wac_b2_n) %>% unlist -> Lubia_R$Wac_b2_n
#Wdc_b2_n
Lubia_H_pred_rebollo %>% 
  filter(is.na(`TIPO MASA`)==T|`TIPO MASA`=="N") %>% 
  group_by(PARCELA, INVENTARIO) %>% 
  summarise(Wdc_b2_n = sum(Biomasa_b2_n, na.rm = T)) %>% 
  ungroup %>% select(Wdc_b2_n) %>% unlist -> Lubia_R$Wdc_b2_n

#We_b2_n
Lubia_H_pred_rebollo %>%  
  group_by(PARCELA, INVENTARIO) %>% 
  summarise(We_b2_n = sum(Biomasa_b2_n_e, na.rm = T)) %>% 
  ungroup %>% select(We_b2_n) %>% unlist -> Lubia_R$We_b2_n

#WT
Lubia_R %>% 
  mutate(WacT = sum(Wac_s_7, Wac_b2_7, Wac_b2_n),
         WdcT = sum(Wdc_s_7, Wdc_b2_7, Wdc_b2_n),
         WeT = sum(We_s_7, We_b2_7, We_b2_n)) -> Lubia_R
...
Lubia_P[is.na(Lubia_P)] <- 0
Lubia_P %>% round(3) -> Lubia_P

Lubia_R[is.na(Lubia_R)] <- 0
Lubia_R %>% round(3) -> Lubia_R
...
Lubia_H_pred_pino %>% 
  ...
  summarise %>% ungroup %>% 
  select(TRATAMIENTO) %>%
  add_column(Lubia_P,., .after = "INVENTARIO") -> Lubia_P
...
Lubia_H_pred_rebollo %>% 
  ...
  summarise %>% ungroup %>% 
  select(TRATAMIENTO) %>%
  add_column(Lubia_R,., .after = "INVENTARIO") -> Lubia_R

#añadir de sitio y especie a pino
Lubia_P %>% add_column(Sitio = "Lubia", Especie = "Pinus pinaster", .before = "PARCELA") -> Lubia_P

#añadir de sitio y especie a rebollo
Lubia_R %>% add_column(Sitio = "Lubia", Especie = "Quercus pirenaica", .before = "PARCELA") -> Lubia_R

source("Lubia_pino_3_tablas.r")
...
source("Lubia_rebollo_3_tablas.r")
...
#write_csv2(Lubia_P, "Lubia_P.csv")
#write_csv2(Lubia_R, "Lubia_R.csv")

#write_csv2(Lubia_H_pred_pino, "Lubia_H_pred_pino.csv")
#write_csv2(Lubia_H_pred_rebollo, "Lubia_H_pred_rebollo.csv")
